...
/*
2d convolution to apply a smoothing filter.
...
/* Place the filter itself in constant memory */
__constant__ float filter_d[filter_width*filter_height];

__global__ void convolution_kernel_naive(float *output, float *input) {
    ...
    if(x<image_width && y<image_height){
            ...
            for (int i=0; i < filter_height; i++) {
                for (int j=0; j < filter_width; j++) {
                    output[y*image_width+x] += input[(y+i)*input_width+x+j] * filter_d[i*filter_width+j];
                }
            }
	    output[y*image_width+x] /= 35;
    }
...
void convolutionCUDA(float *output, float *input, float *filter) {
    ...
    kernelTime.start();
    convolution_kernel_naive<<<grid, threads>>>(d_output, d_input);
...
//THis is specific for a W==H smoothening filter, where W and H are odd.
...
//end initialization
    cudaMemcpyToSymbol(filter_d, filter, filter_width*filter_height*sizeof(float));
